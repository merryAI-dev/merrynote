...
## Hosted Deployment / 웹 배포

MerryNote can now run in a `hosted control room` mode for notes, vocab, and stats.

- Recommended first target: `Render`
- Included Blueprint: [render.yaml](./render.yaml)
- Strategy doc: [DEPLOYMENT.md](./DEPLOYMENT.md)

Hosted mode intentionally disables macOS-only features such as Voice Memos recording control, `launchctl` daemon control, and server-local file path transcription.
