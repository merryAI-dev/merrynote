#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# yapnotesd.sh — yapnotes 백그라운드 데몬
# LaunchAgent에 의해 실행됨. 로그인 시 자동 시작, 크래시 시 자동 재시작.
#
# 감시 폴더:
#   - iCloud yapnotes-inbox/  (iPhone iCloud 연동)
#   - ~/Downloads/             (AirDrop 자동 수신)
#   - EXTRA_WATCH_DIR          (설치 시 설정한 추가 폴더, 선택)
# ─────────────────────────────────────────────────────────────────────

DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
YAPNOTES_ROOT="$(dirname "$DAEMON_DIR")"
SCRIPTS_DIR="$YAPNOTES_ROOT/scripts"
PROMPTS_DIR="$YAPNOTES_ROOT/prompts"
VOCAB_DIR="$YAPNOTES_ROOT/vocab"

# ── 설정 파일 로드 ──────────────────────────────────────────────────
CONFIG_FILE="$HOME/.yapnotes/config.json"
LOG_FILE="$HOME/.yapnotes/yapnotes.log"
mkdir -p "$HOME/.yapnotes"

# config.json에서 값 읽기 (없으면 기본값)
read_config() {
...
import json
try:
    v = json.load(open('$CONFIG_FILE')).get('$key')
...
# ── 감시 폴더 목록 구성 ─────────────────────────────────────────────
ICLOUD_INBOX="$HOME/Library/Mobile Documents/com~apple~CloudDocs/yapnotes-inbox"
DOWNLOADS_DIR="$HOME/Downloads"

mkdir -p "$ICLOUD_INBOX"
WATCH_DIRS=("$ICLOUD_INBOX")

# python3 출력에서 개행/공백 제거 후 비교
...
collect_training_data() {
    ...
    local TD="$HOME/.yapnotes/training-data"
...
process_file() {
    ...
    # .processed 중복 처리 방지
    local PROCESSED_FILE="$(dirname "$FILE")/.processed"
    ...
    if [ "$DURATION_MIN" -gt 5 ]; then
        osascript -e "display notification \"$BASENAME (${DURATION_MIN}분) 전사 시작 — 시간이 좀 걸릴 수 있어요\" with title \"yapnotes 🎙️\"" 2>/dev/null || true
    else
        osascript -e "display notification \"$BASENAME 전사 시작...\" with title \"yapnotes 🎙️\"" 2>/dev/null || true
    fi

    # iCloud 다운로드 완료 대기 (최대 30초)
    local wait=0
    ...
    local ERR_FILE
    ERR_FILE=$(mktemp /tmp/yapnotes-err-XXXXXX)
    ...
    if ! TRANSCRIPT=$(bash "$SCRIPTS_DIR/transcribe.sh" "$FILE" 2>"$ERR_FILE"); then
        ...
        osascript -e "display notification \"$BASENAME 전사 실패 ❌\" with title \"yapnotes\"" 2>/dev/null || true
        return 1
    fi
    rm -f "$ERR_FILE"

    [ -z "$TRANSCRIPT" ] && { log "❌ 전사 결과 없음: $BASENAME"; return 1; }
    ...
    if [ -n "$CLAUDE_PATH" ]; then
        ...
        if [ "$SUMMARY_VALID" = true ]; then
            ...
            # 학습 데이터 수집 (실패해도 메인 파이프라인에 영향 없음)
    ...
    # ── 파일명 개선: Voice Memos 기본 이름 → 마크다운 제목 ────────────
    ...
    # ── 처리 완료 → iCloud inbox 파일 이동 ────────────────────────────
    local FILE_DIR
    FILE_DIR="$(dirname "$FILE")"
    if [[ "$FILE_DIR" == *"yapnotes-inbox"* ]]; then
    ...
    osascript -e "display notification \"$BASENAME → 회의록 완성 ✅\" with title \"yapnotes\" subtitle \"$(basename "$OUTPUT_FILE")\"" 2>/dev/null || true
}
...
| while IFS= read -r -d '' CHANGED_FILE; do
    ...
    # Downloads 폴더의 경우: 이미 처리한 파일 중복 방지 (10분 이내 생성 파일만)
    if [[ "$CHANGED_FILE" == "$DOWNLOADS_DIR"/* ]]; then
        FILE_MTIME=$(stat -f "%m" "$CHANGED_FILE" 2>/dev/null || echo 0)
        FILE_NOW=$(date +%s)
        [ $((FILE_NOW - FILE_MTIME)) -gt 600 ] && continue  # 10분 지난 파일 무시
    fi
